...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        fixtures_dir = os.path.join(settings.APPS_DIR, "base", "fixtures")
        fixture_file = os.path.join(fixtures_dir, "data.json")

        print("Copying media files to configured storage...")
        local_storage = FileSystemStorage(os.path.join(fixtures_dir, "files"))
        ...
        print("Awesome. Your data is loaded! Let's get to it...")
